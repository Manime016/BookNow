...
def create_event_seat(
    db: Session,
    event_seat_data: EventSeatCreate
):
    event = (
        db.query(Event)
        .filter(Event.id == event_seat_data.event_id)
        .first()
    )
    ...
    seat = (
        db.query(Seat)
        .filter(Seat.id == event_seat_data.seat_id)
        .first()
    )
    ...
    existing = (
        db.query(EventSeat)
        .filter(
            EventSeat.event_id == event_seat_data.event_id,
            EventSeat.seat_id == event_seat_data.seat_id
        )
        .first()
    )
    ...
    event_seat = EventSeat(
        event_id=event_seat_data.event_id,
        seat_id=event_seat_data.seat_id,
        price=event_seat_data.price,
        status="available"
    )
